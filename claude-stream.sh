#!/bin/bash
# Wrapper for claude -p that shows tool usage in real-time
# Usage: ./claude-stream.sh "your prompt here"

# Load nvm
export NVM_DIR="${NVM_DIR:-$HOME/.nvm}"
[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"
nvm use --lts --silent 2>/dev/null || true

# Colors
CYAN='\033[0;36m'
GREEN='\033[0;32m'
NC='\033[0m'

claude --verbose --output-format stream-json -p "$@" 2>&1 | while IFS= read -r line; do
    # Skip non-JSON lines (like bash_profile errors)
    if ! echo "$line" | jq -e '.' >/dev/null 2>&1; then
        continue
    fi

    TYPE=$(echo "$line" | jq -r '.type // empty')

    case "$TYPE" in
        assistant)
            # Check for tool use
            TOOL=$(echo "$line" | jq -r '.message.content[0].name // empty')
            if [ -n "$TOOL" ]; then
                if [ "$TOOL" = "TodoWrite" ]; then
                    echo -e "${CYAN}→ ${TOOL}${NC}:"
                    echo "$line" | jq -r '.message.content[0].input.todos[] | "  [\(.status)] \(.content)"' 2>/dev/null
                else
                    DESC=$(echo "$line" | jq -r '.message.content[0].input.description // .message.content[0].input.file_path // .message.content[0].input.command // .message.content[0].input.pattern // ""' 2>/dev/null | head -c 60)
                    echo -e "${CYAN}→ ${TOOL}${NC}: ${DESC}"
                fi
            fi

            # Check for text response
            TEXT=$(echo "$line" | jq -r '.message.content[0].text // empty')
            if [ -n "$TEXT" ]; then
                echo ""
                echo -e "${GREEN}$TEXT${NC}"
            fi
            ;;
        result)
            # Final result - skip since we already showed text above
            ;;
        system)
            # Init message - skip
            ;;
    esac
done
